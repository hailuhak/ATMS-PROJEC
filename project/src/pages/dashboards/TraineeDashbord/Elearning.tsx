...
export const Elearning: React.FC = () => {
  ...
  useEffect(() => {
    const fetchElearningModules = async () => {
      ...
      try {
        ...
        const modulesData: ElearningModule[] = materialsSnap.docs
          .filter((doc) => {
            const data = doc.data();
            return data.courseId && enrolledCourseIds.includes(data.courseId);
          })
          .map((doc) => {
            const data = doc.data();
            return {
        ...
        console.error('Error fetching e-learning modules:', error);
      } finally {
        setLoading(false);
      }
    };
          ...
          <CardContent>
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                No e-learning modules available. Enroll in courses to access learning materials.
              </p>
            </div>
          </CardContent>
        </Card>
