...
export const UserManagement: React.FC = () => {
 ...
 const handleAddUser = async () => {
  ...
  try {
    ...
    // ✅ Build user object (always pending in `users`)
    const userData = {
      uid,
      displayName,
      email,
      role: "pending", // stored as pending in `users`
      createdAt: new Date(),
      lastLogin: new Date(),
    ...
    // ✅ Save chosen role in `pendingUsers` (for admin approval later)
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      // Update Firebase Auth email if current user
        ...
        <Card>
          <CardContent className="space-y-4">
            ...
            <select
              value={newUser.role}
              onChange={e =>
                setNewUser({ ...newUser, role: e.target.value as "trainee" | "trainer" | "admin" })
              }
        ...
        <Card>
          <CardContent className="space-y-4">
            ...
            <select
              ...
              onChange={e =>
                setEditingUser({ ...editingUser, role: e.target.value as "trainee" | "trainer" | "admin" })
              }
              className="px-3 py-2 border border-gray-300 rounded-lg dark:bg-gray-700 dark:text-white"
            >
              <option value="trainee">Trainee</option>
